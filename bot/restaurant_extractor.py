...
# ---------------------------------------------------------------------------
# Shared category list (based on tabelog.com/cat_lst/)
# Used in both Step1 and Step2 prompts to normalize category values.
# ---------------------------------------------------------------------------
_CATEGORY_LIST = (
    "寿司・回転寿司、海鮮・刺身、うなぎ、天ぷら、とんかつ・揚げ物、焼き鳥・串焼き、"
    "すき焼き、しゃぶしゃぶ、そば、うどん、お好み焼き・たこ焼き、丼・定食、おでん、"
    "和食・日本料理、郷土料理・沖縄料理、洋食・ハンバーグ、ステーキ・鉄板焼き、"
    "フレンチ・ビストロ、イタリアン・パスタ・ピザ、スペイン料理、アメリカ料理・ハンバーガー、"
    "中華料理、台湾料理、飲茶・点心、餃子、韓国料理、タイ料理、ベトナム料理、"
    "インド料理、カレー、スープカレー、エスニック料理、焼肉、ホルモン、ジンギスカン、"
    "鍋・もつ鍋、居酒屋、ダイニングバー、立ち飲み・バル、ビアガーデン・ビアホール、"
    "ラーメン、つけ麺、担々麺・油そば、カフェ・喫茶店、甘味処、スイーツ・洋菓子、"
    "パン・ベーカリー、バー・ワインバー、弁当・惣菜・デリ、ビュッフェ、"
    "創作料理・イノベーティブ、その他"
)
...
- `category`: 必ず以下のリストから最も近いものを1つ選ぶ。どれにも当てはまらない場合は「その他」。
  {_CATEGORY_LIST}
...
async def _fetch_url_content(url: str) -> Optional[str]:
    ...
    try:
        ...
        logger.warning(
            "URL fetch blocked (HTTP %d) for %s — enrichment skipped",
            e.response.status_code, domain,
        )
        return None
    except httpx.TimeoutException:
        logger.warning("URL fetch timed out for %s — enrichment skipped", domain)
        return None
    except Exception as e:
        logger.warning("URL fetch failed for %s: %s — enrichment skipped", domain, e)
        return None
